...
using Neko_Love.Net.V1.Responses;

namespace Neko_Love.Net.Tests;
...
public class NekoV1ClientTest
{
    ...
    public async Task RequestSingleEndpointTest()
    {
        ...
        results.AddRange(await _client.RequestNsfwResultsAsync(NsfwEndpoint.Nekolewd));
    ...
    {
        List<NekoImage> results = new();
        results.AddRange(await _client.RequestNsfwResultsAsync(NsfwEndpoint.Random | NsfwEndpoint.All | NsfwEndpoint.Nekolewd));
        var nsfwValidFlagCount = Enum.GetValues<NsfwEndpoint>().Length - 2;
